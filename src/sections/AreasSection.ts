 ...
 * HA doesn't provide a default icon for floors, but these are commonly used in custom floor plans and make a nice fallback.
 */
function getFloorIcon(level: number | null): string {
  if (level === null) return 'mdi:floor-plan';
  else if (level == -1) return 'mdi:home-floor-negative-1';
  else if (level >= 0 && level <= 3) return `mdi:home-floor-${level}`;
