...
- **Kamera-Ansicht (CCTV)** — Opt-in View mit allen Kameras: Scheinwerfer-Steuerung, PTZ-Steuerkreuz und Aufnahmen-Link (Reolink), LLM Vision Ereignis-Timelines *(ab v1.4.0)*
...
**Aktivitäts-Verlauf in der Sicherheits-Ansicht** *(ab v1.4.0)* — Wie im Home Assistant Security-Panel zeigt ein 24-Stunden-Logbuch die Aktivität aller Sicherheits-Entitäten und Personen. Darstellung wählbar: als **normale Section** am Ende (Standard — die übrigen Sections nutzen die volle Breite) oder als **Seitenleiste** rechts angepinnt (eigener Tab auf schmalen Bildschirmen, belegt dafür die rechte Spalte; `security_activity_layout: sidebar`). Standardmäßig an; entfällt automatisch, wenn die Logbuch-Integration nicht geladen ist, und lässt sich per `show_security_activity: false` abschalten. Einzelne Kameras lassen sich zusätzlich **nur für die Sicherheits-Ansicht** abwählen (`security_hidden_cameras`, Checkboxen im Editor) — Raum- und Kamera-Ansicht bleiben davon unberührt.

**Kameras in der Sicherheits-Ansicht** *(ab v1.4.0)* — Mit `show_cameras_in_security` erscheinen die Kameras als kompakte Standbild-Karten in der Sicherheits-Ansicht — dem Vorbild von Home Assistants Security-Panel folgend (dort gehören Kameras zu Alarm, Schlössern und Kontakten). Mit `group_security_by_areas` lässt sich die Sicherheits-Ansicht alternativ **nach Bereichen** gliedern statt nach Kategorien: pro Bereich eine Section, die Überschrift führt per Tap in die jeweilige Raum-Ansicht. Einzelne Kameras blendet man wie gewohnt aus (Label `no-dboard` oder die Kamera-Gruppe im Bereichs-Editor — wirkt überall).
...
| `group_security_by_areas` | boolean | `false` | v1.4.0 | Sicherheits-Ansicht nach Bereichen statt Kategorien gliedern (Überschrift führt in die Raum-Ansicht) |
...
| `security_activity_layout` | `sidebar` \| `section` | `section` | v1.4.0 | Aktivitäts-Verlauf als normale Section am Ende (Standard) oder rechts angepinnt (Seitenleiste) |
| `security_hidden_cameras` | string[] | `[]` | v1.4.0 | Kameras nur aus der Sicherheits-Ansicht ausblenden (Checkboxen im Editor; Raum-/Kamera-Ansicht unberührt) |
