...
- [🖥️ Benutzung / Dashboard erstellen](#️-benutzung--dashboard-erstellen)
- [🎨 Grafischer Konfigurator (Editor)](#-grafischer-konfigurator-editor)
...
- **Debug-Einstellungen** - Log-Level konfigurierbar (error, warn, info, debug)
...
| `use_alarmo_card` | boolean | `false` | Verwendet die Alarmo-Karte statt der Standard-Tile-Karte (nur bei Alarmo-Entities) |
| `show_scheduler_card` | boolean | `false` | Zeigt die Scheduler-Karte in der Übersicht an |
| `scheduler_entity` | string | `''` | Entity-ID der Scheduler-Entity (z.B. `switch.scheduler_heating`) |
| `show_calendar_card` | boolean | `false` | Zeigt die Calendar-Karte in der Übersicht an |
| `use_calendar_card_pro` | boolean | `false` | Verwendet Calendar Card Pro statt der Standard-Kalender-Karte |
| `calendar_entities` | array | `[]` | Array von Kalender-Entity-IDs (z.B. `['calendar.home', 'calendar.work']`) |
...
- **Automatische Bereinigung:** Nach der Pattern-Anwendung werden possessive Präfixe (z.B. "Phillipp's ") automatisch entfernt, falls vorhanden. Dies hilft bei Namen wie "Socket - Phillipp's PC", die nach dem Pattern "^.* - " zu "PC" werden.

**Domain-spezifische Patterns:**
Du kannst Patterns auf bestimmte Entity-Typen beschränken, indem du ein Objekt mit `pattern` und `domain` verwendest:
...
#### `entity_name_translations` - Entity-Namen und Area-Namen Übersetzungen

Mit `entity_name_translations` kannst du Substrings in Entity-Namen und Area-Namen übersetzen. Dies ist besonders nützlich, wenn Entities oder Areas englische Namen haben, die du ins Deutsche übersetzen möchtest, oder umgekehrt.

**Im Editor:** Die Entity-Name-Übersetzungen können im Editor unter "Entity Name Translations" konfiguriert werden.
...
und du die englischen Begriffe ins Deutsche übersetzen möchtest (wenn das Dashboard auf Deutsch eingestellt ist):
...
**Hinweis:** Die Übersetzung wird nur auf die angezeigten Namen im Dashboard angewendet, nicht auf die tatsächlichen Entity-Namen oder Area-Namen in Home Assistant. Übersetzungen werden als ganze Wörter behandelt (mit Wortgrenzen), sodass "Bedroom" nicht "BedroomLight" ersetzt. Übersetzungen sind sprachabhängig - sie werden nur angewendet, wenn die Dashboard-Sprache mit der Zielsprache (`to_lang`) übereinstimmt. Übersetzungen gelten sowohl für Entity-Namen als auch für Area-Namen (z.B. View-Titel).
...
Das Dashboard unterstützt verschiedene optionale Integrationen, die alle über den Editor konfiguriert werden können:

- **Search Card** - Zeigt eine Suchfunktion in der Übersicht
- **Alarm Panel** - Integration von Alarmanlagen in die Security-View
- **Reolink Kameras** - Erweiterte Unterstützung für Reolink-Kameras
- **Better Thermostat** - Ersetzt Standard-Thermostat-Karten durch Better Thermostat UI-Karten
...
<details>
<summary><strong>🔌 Details zu optionalen Integrationen</strong></summary>
...
<details>
<summary>YAML-Konfiguration</summary>
